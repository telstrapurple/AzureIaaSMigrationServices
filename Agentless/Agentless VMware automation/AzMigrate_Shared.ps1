class AzMigrate_Shared
{
    ...
    [psobject] GetDiscoveredServer([string] $AzMigrateResourceGroupName,
                                        ...
                                        [string] $AzMigrateSubscriptionId,
                                        [string] $machineName)
    {
        #region Validation-Errorlogging
        if ([string]::IsNullOrEmpty($AzMigrateSubscriptionId)) {
            $this.Logger.LogError("Azure Migrate SubscriptionId is not specified")    
            return $null
        }
        ...
        $DiscoveredServerList = Get-AzMigrateDiscoveredServer -ProjectName $AzMigrateProjectName `
                                                        ...
                                                        -SubscriptionId $AzMigrateSubscriptionId `
                                                        -DisplayName $machineName

        if (-not $DiscoveredServerList)
        {
            $this.Logger.LogTrace("Discovery Details for the Server could not be retrieved for '$($AzMigrateSubscriptionId)-$($AzMigrateResourceGroupName)-$($AzMigrateProjectName)-$($machineName)'")
        }
                                        ...
                                        [string] $machineName)
    {
        ...
        $DiscoveredServerMachine = $this.GetDiscoveredServer($AzMigrateResourceGroupName, $AzMigrateProjectName, $AzMigrateSubscriptionId, $machineName)
