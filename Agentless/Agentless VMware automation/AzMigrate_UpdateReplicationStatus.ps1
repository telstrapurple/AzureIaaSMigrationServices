...
Function ProcessItemImpl($processor, $csvItem, $reportItem) {    
    ...
    $AzMigrateSubscriptionId = $csvItem.AZMIGRATEPROJECT_SUBSCRIPTION_ID
    if ([string]::IsNullOrEmpty($AzMigrateSubscriptionId)) {
    ...
    $ReplicatingServermachine = $AzMigrateShared.GetReplicationServer($azMigrateRG, $azMigrateProjName, $AzMigrateSubscriptionId, $sourceMachineName)
    if (-not $ReplicatingServermachine)
    {
        $this.Logger.LogError("Azure Migrate Replicating Server could not be retrieved for  for '$($AzMigrateSubscriptionId)-$($azMigrateRG)--$($azMigrateRG)-$($azMigrateProjName)-$($sourceMachineName)'")
        $reportItem.AdditionalInformation = "Azure Migrate Replicating Server could not be retrieved for  for '$($AzMigrateSubscriptionId)-$($azMigrateRG)--$($azMigrateRG)-$($azMigrateProjName)-$($sourceMachineName)'"
        return
    }
    ...
    try {
        ...
        $this.Logger.LogError("Azure Migrate jobs could not be retrieved for  for '$($AzMigrateSubscriptionId)-$($azMigrateRG)--$($azMigrateRG)-$($azMigrateProjName)-$($sourceMachineName)'")
        $reportItem.AdditionalInformation = "Azure Migrate jobs could not be retrieved for  for '$($AzMigrateSubscriptionId)-$($azMigrateRG)--$($azMigrateRG)-$($azMigrateProjName)-$($sourceMachineName)'"
        return
    }
    ...
    if (-not $JobByid)
    {
        $this.Logger.LogError("Azure Migrate jobs could not be retrieved by JobId for  for '$($AzMigrateSubscriptionId)-$($azMigrateRG)--$($azMigrateRG)-$($azMigrateProjName)-$($sourceMachineName)'")
        $reportItem.AdditionalInformation = "Azure Migrate jobs could not be retrieved for  for '$($AzMigrateSubscriptionId)-$($azMigrateRG)--$($azMigrateRG)-$($azMigrateProjName)-$($sourceMachineName)'"
        return
    }
    ...
    if ($JobByid.State -eq 'Failed') {
        ...
        $processor.Logger.LogTrace("ReplicationJob is active for  for '$($AzMigrateSubscriptionId)-$($azMigrateRG)--$($azMigrateRG)-$($azMigrateProjName)-$($sourceMachineName)'")
    }  
}
