...
Function ProcessItemImpl($processor, $csvItem, $reportItem) {
    ...
    $azSubscriptionId = $csvItem.TARGET_SUBSCRIPTION_ID
    if ([string]::IsNullOrEmpty($azMigrateProjName)) {
    ...
    if ([string]::IsNullOrEmpty($targetResourceGroup)) {
        ...
        if ($azSubscriptionId){ 
            ...
            $params.Add("SubscriptionId", $AzMigrateSubscriptionId)
        }
    ...
    $DiscoveredServer = $AzMigrateShared.GetDiscoveredServer($azMigrateRG, $azMigrateProjName, $AzMigrateSubscriptionId, $sourceMachineName)
    ...
    if ($DiscoveredServer) {
        ...
        if (($csvItem.OK_TO_USE_ASSESSMENT -ne 'Y')) {
            ...
            if (-not (([string]::IsNullOrEmpty($azMigrateAssessmentName)) -or ([string]::IsNullOrEmpty($azMigrateGroupName))))
            {
                ...
                if ($AzMigrateSubscriptionId) {
                    $MigPrj = Get-AzMigrateProject -Name $azMigrateProjName -ResourceGroupName $azMigrateRG -SubscriptionId $AzMigrateSubscriptionId
                }
                else {
